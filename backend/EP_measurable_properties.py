...
def ep1(event_types_to_table_df_map, events_to_time_df, samp_rate):
    ...
    def ep1_iter(event_type, df):
            ...
            df[ts_id] = 1
            df = df[['assign_mech_time', ts_id]]
            df = df.set_index('assign_mech_time')
            ts = df[ts_id]
            ts = ts.resample(samp_rate).sum()
            return ts
...
def ep2(event_types_to_table_df_map, events_to_time_df, agg_mode, samp_rate):
    ...
    def ep2_iter(event_type, df):
        ...
        for event_attribute in event_attributes:
            if is_any_real_numeric_dtype(df[event_attribute]):
                ...
                attr_df = df[['assign_mech_time', event_attribute]]
                attr_df = attr_df.rename(columns={event_attribute: ts_id})
                attr_df = attr_df.set_index('assign_mech_time')
                ts = attr_df[ts_id]
                attr_ts_dict[event_attribute] = agg(ts, agg_mode, samp_rate)
    ...
    ep2_dict = {event_type:{} for event_type in event_types_to_table_df_map.keys()}
    for event_type, event_type_df in event_types_to_table_df_map.items():
...
def ep3(event_object_count_df_map, events_to_time_df, agg_mode, samp_rate):
...
def ep4(event_object_combinations, event_object_count_df_map, events_to_time_df, agg_mode, samp_rate):
